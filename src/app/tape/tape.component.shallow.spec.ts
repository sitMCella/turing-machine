...
describe("TapeComponent", () => {
  class FakeFirstOperation implements Operation {
    apply(machineStatus: MachineStatus): MachineStatus {
      const finalSquares: Array<Square> = [
        new Square(1, new TapeSymbol(TapeSymbol.ONE)),
        new Square(2, new TapeSymbol(TapeSymbol.NONE)),
      ];
      const finalTape: Tape = new Tape(finalSquares);
      const finalMachineStatus: MachineStatus = new MachineStatus(
        machineStatus.configurationName,
        finalTape,
        1,
      );
  ...
  class AlgorithmService implements Algorithm {
    ...
    public evolve(initialTape: Tape): Observable<MachineStatus> {
      const initialStatus: MachineStatus = new MachineStatus(
        "b",
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      imports: [FormsModule],
      providers: [{ provide: IntervalService, useValue: intervalService }],
    }).compileComponents();
  }));
  ...
  it("should contain the first configuration name", async(() => {
    ...
    const firstConfigurationName: HTMLElement = compiled.querySelector(
      ".initial-tape-configuration .configuration-name",
    );
  ...
  it("should contain the default initial tape", async(() => {
    ...
    const initialTape: HTMLElement = compiled.querySelector(
  ...
  it("should contain the default initial tape with 2 squares", async(() => {
    ...
    const squares: Array<HTMLElement> = compiled.querySelectorAll(
      ".initial-tape-configuration .square",
    );
  ...
  describe("resize setting", () => {
    it("should resize default initial tape squares count", async(() => {
      ...
      const squaresCount: HTMLInputElement = compiled.querySelector(
        ".squaresCount input",
      );
      ...
      fixture.whenStable().then(() => {
        const resizeButton: HTMLButtonElement =
          compiled.querySelector(".applySettings");
        resizeButton.click();
        fixture.detectChanges();
        algorithmTimeService.tick(1);
        fixture.detectChanges();
  ...
  describe("evolve button", () => {
    it("should evolve algorithm with default initial tape", async(() => {
      ...
      const tapes: Array<any> = compiled.querySelectorAll(
        ".complete-configuration",
      );
      expect(tapes).not.toBeNull();
      expect(tapes).toBeDefined();
      expect(tapes.length).toBe(2);
      const secondTapeSquares: Array<HTMLElement> =
        tapes[1].querySelectorAll(".square");
    ...
    it("should evolve algorithm with modified initial tape", async(() => {
      ...
      const firstSquareInitialTape: HTMLInputElement =
        squares[0].querySelector("input");
      firstSquareInitialTape.value = "1";
      firstSquareInitialTape.dispatchEvent(new Event("input"));
      fixture.detectChanges();
      fixture.whenStable().then(() => {
        const evolveButton: HTMLButtonElement =
          compiled.querySelector(".evolve");
        evolveButton.click();
        fixture.detectChanges();
        algorithmTimeService.tick(1);
        fixture.detectChanges();
