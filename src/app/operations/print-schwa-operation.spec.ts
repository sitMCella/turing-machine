...
describe('PrintSchwaOperation', () => {
  ...
  it('should the new machine status be the same object as the initial machine status', () => {
    ...
    const newMachineStatus: MachineStatus = printSchwaOperation.apply(machineStatus);
  ...
  it('should throw Error if tape already contains a symbol in current index', () => {
    const squaresError: Array<Square> = [new Square(1, new TapeSymbol(TapeSymbol.ONE))];
    const tapeError: Tape = new Tape(squaresError);
    const machineStatusError: MachineStatus = new MachineStatus(configurationName, tapeError, 0);
