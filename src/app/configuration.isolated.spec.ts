...
describe("Configuration", () => {
  ...
  class FakeSecondOperation implements Operation {
    apply(machineStatus: MachineStatus): MachineStatus {
      const finalSquares: Array<Square> = [
        new Square(1, new TapeSymbol(TapeSymbol.ONE)),
        new Square(2, new TapeSymbol(TapeSymbol.ZERO)),
      ];
      const finalTape: Tape = new Tape(finalSquares);
      const finalMachineStatus: MachineStatus = new MachineStatus(
        machineStatus.configurationName,
  ...
  const initialMachineStatus: MachineStatus = new MachineStatus(
    initialConfigurationName,
    tape,
    0,
  );
  ...
  it("should evolve machine status with one operation", () => {
    ...
    const configuration: Configuration = new Configuration(
      ...
      symbol,
      operations,
      finalConfigurationName,
    );

    const finalMachineStatus: MachineStatus =
      configuration.evolve(initialMachineStatus);
    ...
    expect(finalMachineStatus.tape.squares[0].symbol.value).toBe(
      TapeSymbol.ONE,
    );
    expect(finalMachineStatus.tape.squares[1].id).toBe(2);
    expect(finalMachineStatus.tape.squares[1].symbol.value).toBe(
      TapeSymbol.NONE,
    );
  ...
  it("should evolve machine status with two operations", () => {
    const operations: Array<Operation> = [
      new FakeFirstOperation(),
      new FakeSecondOperation(),
    ];
    const symbol: TapeSymbol = new TapeSymbol(TapeSymbol.NONE);
    const configuration: Configuration = new Configuration(
      initialConfigurationName,
