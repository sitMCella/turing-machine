...
export class IncreasingOnesAlgorithmService implements Algorithm {
  ...
  constructor(private algorithmEvolutionService: AlgorithmEvolutionService) {
    ...
    const fourthConfiguration = new Configuration(
      ...
      [new MoveRightOperation(), new MoveRightOperation()],
      "q",
    );
    const fifthConfiguration = new Configuration(
      "q",
      new TapeSymbol(TapeSymbol.NONE),
      [new PrintOneOperation(), new MoveLeftOperation()],
      "p",
    );
    const sixthConfiguration = new Configuration(
      "p",
      new TapeSymbol(TapeSymbol.X),
      [new EraseOperation(), new MoveRightOperation()],
      "q",
    );
    const seventhConfiguration = new Configuration(
      "p",
      new TapeSymbol(TapeSymbol.SCHWA),
      [new MoveRightOperation()],
      "f",
    );
    const eightConfiguration = new Configuration(
      "p",
      new TapeSymbol(TapeSymbol.NONE),
      [new MoveLeftOperation(), new MoveLeftOperation()],
      "p",
    );
    const ninthConfiguration = new Configuration(
      "f",
      new TapeSymbol(TapeSymbol.ANY),
      [new MoveRightOperation(), new MoveRightOperation()],
      "f",
    );
    const tenthConfiguration = new Configuration(
      "f",
      new TapeSymbol(TapeSymbol.NONE),
      [
        new PrintZeroOperation(),
